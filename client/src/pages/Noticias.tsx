...
export default function Noticias() {
  ...
  const feedQuery = trpc.news.listFeed.useQuery({ limit: 6, offset });

  const items = useMemo<FeedItem[]>(() => {
    return (feedQuery.data?.items as FeedItem[]) ?? [];
  }, [feedQuery.data]);

  const hasNextPage = feedQuery.data?.hasMore ?? false;
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (feedQuery.data?.nextOffset) {
          setOffset(feedQuery.data.nextOffset);
        }
      },
      {
        root: null,
  ...
  }, [hasNextPage, feedQuery.isLoading, feedQuery.data?.nextOffset]);
